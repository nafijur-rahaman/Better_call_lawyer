  ...
  return (
    <div className="max-w-full xl:max-w-[1200px] 2xl:max-w-[1500px] mx-auto ">
      {chartData.length > 0 ? (
        <>
          ...
          <div className="lawyer-booking-container border border-[#0F0F0F30] rounded-xl p-8 mb-20 mt-10">
            ...
            {lawyer.map((data, index) => (
              <div
                key={index}
                className="app-card border border-[#0F0F0F30] rounded-xl p-5 mt-5"
              >
        ...
        <div className="chart-container flex flex-col justify-center items-center border p-5 mb-20 border-[#14141420] rounded-lg">
          <h1 className="text-2xl font-bold text-center">
            No Appointment Found for toady
          </h1>
          <p>Please come back tomorrow</p>
        <Link to={'/'}>
          <button className="text-[#0EA106] border border-[#0EA106] w-full px-5 py-2 rounded-3xl cursor-pointer font-semibold mt-5">Book Appointment</button>
        </Link>
        </div>
      )}
    </div>
